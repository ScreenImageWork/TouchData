...
public class RequestListenerThread extends Thread
{
    ...
    public void destroy()
    {
        try
        {
            TPLog.printKeyStatus("RequestListenerThread destroy");
    ...
    public void run()
    {
        TPLog.printKeyStatus("RequestListenerThread start...");
        ...
        while (!Thread.interrupted())
        {
            ...
            {
                TPLog.printError("RequestListenerThread 运行时出现异常：");
                TPLog.printError(e);
                this.interrupt();
            }
        }
    ...
    private void initHttpServer()
    {
        try
        {
            TPLog.printKeyStatus("HttpServer init ，port="+port+",webRoot="+webRoot);
...
//            registry.register(Constant.Http.BROWSE, new FileBrowseHandler(webRoot));
            registry.register(Constant.Http.BROWSE, new TPHttpHandler()); // 2017.03.09 by zl

            httpService = new HttpService(httpProcessor,
                new DefaultConnectionReuseStrategy(), new DefaultHttpResponseFactory());
            httpService.setParams(params);
            httpService.setHandlerResolver(registry);//为http服务设置注册好的请求处理器。
            TPLog.printKeyStatus("HttpServer init completed！");
        }
        catch (IOException e)
        {
            TPLog.printError("HttpServer init Exception：");
            TPLog.printError(e);
        }
    }
