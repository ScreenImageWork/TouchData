...
public class SavePageUtils_bk {
	...
	private static SavePageUtils_bk mSavePageUtils;
	...
	private SavePageUtils_bk(){

	}

	public synchronized static SavePageUtils_bk getInstence(){
		if(mSavePageUtils==null){
			mSavePageUtils = new SavePageUtils_bk();
		}
...
//					float oy = mSubPage.getOffsetY();
					...
					if(mSubPage!=null){
						...
						if(bounds.width>BITMAP_MAX_WIDTH||bounds.height>BITMAP_MAX_HEIGHT){
							if(bounds.width>BITMAP_MAX_WIDTH&&bounds.height>BITMAP_MAX_HEIGHT){
								float widthScale = BITMAP_MAX_WIDTH / bounds.width;
								float heightScale = BITMAP_MAX_HEIGHT / bounds.height;
								if(widthScale<heightScale){
									boundsScale = widthScale;
								}else{
									boundsScale = heightScale;
								}
							}else if(bounds.width>BITMAP_MAX_WIDTH){
								boundsScale = BITMAP_MAX_WIDTH / bounds.width;
							}else{
								boundsScale = BITMAP_MAX_HEIGHT / bounds.height;
							}

							 width = (int)BITMAP_MAX_WIDTH;
							 height = (int)BITMAP_MAX_HEIGHT;
						}

						float ox = (bounds.x*-1+(width - bounds.width*boundsScale));
						float oy = (bounds.y*-1+(height - bounds.height*boundsScale));


						Matrix matrix = new Matrix();
						matrix.postTranslate(ox,oy);
						matrix.postScale(boundsScale,boundsScale);
...
//						scaleHeight = (int)((float)height*curScale);// + (int)BITMAP_BOUNDS*2;
						...
						TPLog.printError("save img width="+width+",height="+height);

						//1.将图元转化成bitmap
						Bitmap graphBitmap = Bitmap.createBitmap(width, height, Bitmap.Config.ARGB_8888);
						Canvas canvas = new Canvas(graphBitmap);

						if(mSubPage.getBackgroundColor() == WhiteBoardUtils.BACKGROUNDCOLOR[0]){
							BgFactory.drawGriddingBg(canvas,width,height,WhiteBoardUtils.BACKGROUNDCOLOR[1]);
						}else{
						...
						int layerCount = canvas.saveLayer(0,0,(int)width,(int)height,null);
						...
						if(angle!=0) {//rotating img
							...
							TPLog.printError("width="+width,",height="+height);

							//4.进行图片合成，叠加合成
							bitmap = Bitmap.createBitmap(width, height, Bitmap.Config.ARGB_8888);

							float rotatePx = width / 2f;
							float rotatePy = height / 2f;
